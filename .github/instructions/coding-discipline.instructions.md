...
Governing thought: repository applications follow strict architectural patterns—Redux on client, aggregates/projections on server, schema-first generation throughout. Deviations are refused.

> Drift check: Review `src/` for reference patterns; `src/Reservoir/` for client state; `src/DomainModeling.Runtime/` for server patterns.
...
- Code outside these patterns **MUST** be refused or refactored to comply. Why: Architecture consistency is non-negotiable.
- PRs introducing non-aggregate writes, non-Redux client state, or business logic outside Domain **MUST** be rejected. Why: Core projects are reference implementations.
...
All contributors building repository applications with Alfred's Forge. Core projects serve as reference implementations—they demonstrate correct usage and must be exemplary.
...
- **Zero tolerance**: refuse non-compliant code
