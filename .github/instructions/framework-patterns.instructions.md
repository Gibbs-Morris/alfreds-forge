...
- The Domain project pattern demonstrates the goal: almost all business logic testable without Orleans, Cosmos, HTTP, or SignalR. Why: Reference implementation proves the pattern.
...
Contributors building or extending the Alfred's Forge framework under `src/`. This is about building the infrastructure that enables strict and repeatable architectural patterns.
...
- **Testability by design**: business logic testable without infrastructure (see Domain implementations under `src/`)
