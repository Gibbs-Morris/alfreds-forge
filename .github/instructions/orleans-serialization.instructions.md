...
- Types **MUST** include globally unique `[Alias]`; member IDs **MUST** start at 0 per inheritance level and be unique for that level. Why: Supports safe renames and deterministic layout.
- **Pre-1.0 serialization freedom**: While the repository is pre-1.0 (see `.github/instructions/backwards-compatibility.instructions.md`), member IDs and type shapes **MAY** be changed freely; agents **MUST NOT** add compatibility shims for serialization layouts that only exist on the current branch. Why: Pre-release; no rolling deployments to protect.
- **Post-1.0 serialization stability**: Once the repository reaches 1.0+, existing member IDs **MUST NOT** change and **MUST** remain unique/unchanged for their level; breaking type changes (e.g., record-to-class or signedness changes) **MUST NOT** be made; new members **MUST** use unused IDs. Why: Protects rolling deployments.
- Post-1.0, versioning **MUST** follow additive/compatible changes; removals **SHOULD** use `[NonSerialized]`/`[Obsolete]`; widening numeric types and making properties nullable **SHOULD** be preferred over narrowing. Why: Keeps serialized data readable.
