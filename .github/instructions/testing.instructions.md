...
Governing thought: Default to fast, deterministic L0 tests with high coverage and mutation strength; Alfred's Forge projects carry full gates, Samples stay minimal.
...
- Alfred's Forge projects **MUST** run mutation testing and maintain or raise the score; mutation runs **MUST** be allowed to finish (~30 minutes) and mutation scripts **MUST** be preceded by `dotnet tool restore` and a clean build. Why: Mutation score enforces assertion quality.
...
Applies to all test authors across Alfred's Forge and Samples solutions, including mutation work and legacy test improvements.
...
- Add mutation (Alfred's Forge): `pwsh ./eng/src/agent-scripts/test-project-quality.ps1 -TestProject <Name>`
- Build-only during iteration: `dotnet build ./tests/<Name>/<Name>.csproj -c Release -warnaserror`
- Full pipeline (Alfred's Forge): `pwsh ./eng/src/agent-scripts/unit-test-Alfred's Forge-solution.ps1` then `mutation-test-Alfred's Forge-solution.ps1`
...
- **Default behavior**: `unit-test-Alfred's Forge-solution.ps1` and `unit-test-sample-solution.ps1` run L0Tests and L1Tests.
...
- Alfred's Forge vs Samples: Samples showcase patterns (minimal tests, no mutation gate); Alfred's Forge enforces full gates.
...
### Mutation Loop (Alfred's Forge)

1. Build clean, then run `mutation-test-Alfred's Forge-solution.ps1` (or `test-project-quality.ps1` without `-SkipMutation`).
